...
def get_user_input():
    "Get weight and height input from the user with validation."
...
def convert_weight_to_kg(weight_pounds):
    "Convert weight from pounds to kilograms with validation."
...
def convert_height_to_meters(height_feet, height_inches):
    "Convert height from feet and inches to meters with validation."
...
def display_results(bmi_value):
    "Display BMI results with assertion checks."
...
def bmi_table():
    "BMI table with height as columns and weight as rows."
    #Activity 4: BMI table
    heights = list(range(58, 77, 2))  #58 to 76 by 2
    weights = list(range(100, 251, 10))  #100 to 250 by 10

    print("\nBMI Table")
    print("Height (in) top-bottom   Weight (lb) left-right")

    #Prints row with weights
    print(f"{'':<10}", end="")  
    for w in weights:
        print(f"{w:<6}", end="")
    print()

    #Prints rows of BMI values
    for h in heights:
        print(f"{h:<10}", end="")   #height column
        for w in weights:
            weight_kg = convert_weight_to_kg(w)
            height_meters = h * INCHES_TO_METERS
            bmi = calculate_bmi(weight_kg, height_meters)
            print(f"{bmi:<6.1f}", end="")
        print()

def main():
    "Run the BMI calculator with loops and exception handling."
    #Main with exception handling.
    #checks that errors and reports are caught
    #Activity 2:User can now run BMI calculator again after results
    while True:
        weight_pounds, height_feet, height_inches = get_user_input()

        #Activity 2:Quits if user enters "q"
        if None in (weight_pounds, height_feet, height_inches):
